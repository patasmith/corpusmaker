...
class Exporter:
    ...
    def convert_pcp_to_chat_completion_jsonline(self, pcp: dict[str, str]) -> str:
        system_message = self.make_message("system", self.system_prompt.encode("unicode_escape").decode("utf-8"))
        user_message = self.make_message("user", pcp["prompt"])
        assistant_message = self.make_message("assistant", pcp["completion"].encode("unicode_escape").decode("utf-8"))
